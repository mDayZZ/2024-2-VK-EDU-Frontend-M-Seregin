...
const ConversationItem = ({userId, conversation, openChatPage}) => {
    const conversationLastMessage = `${conversation.last_message?.user?.name || conversation.last_message?.user?.username}: ${conversation.last_message?.content}` || 'Сообщений пока нет';
    return (
